...
def main() -> None:
    """Creates ...
    NOTE The keyswords 'format', 'nest' and 'template' in YAML block of md 
    files are automatically removed from 'meta' and used to instruct build 
    ...
    """Extract any 'format', 'nest' and 'template' from meta and sanitize frontmatter."""
    data = [
        {
            "format": item["entry"]["attributes"].get("format", ".template"),
            "nest": item["entry"]["attributes"].get("nest", False),
