...
/* Returns Assets class, optionally extended from parent. */
export const AssetsFactory = (parent = class {}) => {
  return class Assets extends parent {
    static name = "Assets";

    /* NOTE
    - 'Assets' deliberatly provides an API similar to 'Loaders'. It would likely 
      be possible to do more to enforce/exploit (unDRY) this similarity via class 
      composition, increased use of shared functions etc. However, 'Assets'
      and 'Loaders' are kept relatively separate because:
      - 'Assets' is much less critical than 'Loaders' and should not get in the 
        way of optimizing 'Loaders'. In fact, it's likely that 'Assets' will
        only be used in development.
      - Despite the API similarities, 'Assets' and 'Loaders' are very different 
        creatures with different use cases.
      Having said that... 'Assets' is delivered via a factory function, so that
      it can be plugged into Rollo's class composition system, if needed...  */
        ...
        constructor() {
          ...
          this.#cache = new (class Cache {
            ...
            async get(path) {
              ...
              /* Use 'owner' to get text. Ensures that any cached text is used. */
              const text = await owner.import(path, { raw: true });
...
/* Utility for importing public files as per file type or as text (raw). 
NOTE
- Enables truly dynamic imports.
- Supports batch imports.
- Requires pre-built '__manifest__.js'.
- Uses the '/' syntax.
...
export const assets = new (AssetsFactory())();
