...
export const reactive = (parent, config, ...factories) => {
  const cls = class Reactive extends parent {
    ...
    #effects = new (class {
      ...
      add = (effect, condition) => {
        ...
        Reactive.#call_effect(
          effect,
          condition,
          this.#owner.#create_effect_data(...Object.keys(this.#owner.#state_data.current)),
          this.#owner
        );
    ...
    #$ = new Proxy(this, {
      get: (target, key) => {
        return target.#state_data.current[key];
      },
    ...
    /* Returns object with shallowly frozen shallow copies of underlying state data. */
    get state() {
      return { 
        current: Object.freeze({ ...this.#state_data.current }),
        previous: Object.freeze({ ...this.#state_data.previous }),
       };
    }
    ...
    #protected = new (class {
      ...
      add = (key, value) => {
        ...
        const setter = (value) => {
          if (!this.#owner.#is_equal(value, this.#owner.#state_data.current[key])) {
    ...
    clear() {
      this.state_data.previous = this.state_data.current;
      this.state_data.current = {};
      return this;
    }
  ...
  to be passed into effects/conditions. */
    #create_effect_data = (...keys) => {
      ...
      for (const key of keys) {
        data[key] = {
          current: this.#state_data.current[key],
          previous: this.#state_data.previous[key],
        };
      }
      Object.freeze(data);
      return data;
  ...
  different from current is returned. */
    #get_changes = (data) => {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        if (
          !(key in this.#state_data.current) ||
          !this.#is_equal(value, this.#state_data.current[key])
        ) {
          changes[key] = value;
        }
      }
    ...
    #update_stores = (data) => {
      this.#state_data.previous = { ...this.#state_data.current };
      for (const [key, value] of Object.entries(data)) {
        this.#state_data.current[key] = value;
      }
      return this;
    };
  };
