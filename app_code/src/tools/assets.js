...
20250303
src/tools/assets.js
https://testapp8dev.anvil.app/_/api/asset?path=src/tools/assets.js
...
await (async () => {
  ...
  const html = await assets.fetch("foo.html");
  const container = component.div({ parent: app, innerHTML: html });

  const css = await assets.fetch("foo.css");
  Sheet(css, document);

  const { foo } = await assets.import("foo");
  container.insertAdjacentText("beforeend", foo);
})();
