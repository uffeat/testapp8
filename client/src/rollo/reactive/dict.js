...
await (async () => {
  const { Reactive } = await import("rollo/reactive/dict");
...
await (async () => {
  const { Reactive } = await import("rollo/reactive/dict");
  const state = Reactive({ foo: 42 });
  state.effects.add(
    ({ current, owner, self, previous, added, removed, updated }) => {
